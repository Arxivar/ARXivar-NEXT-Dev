...
namespace ExampleApplication
{
    public partial class Form1 : Form
    {
        ...
        private void buttonGetAooAsync_Click(object sender, EventArgs e)
        {
            try
            {
                //Inizializziamo "BusinessUnit & DocumentTypes Api
                var aooApi = new ... . ... . ... . ...(Configuration);
                var docTypesApi = new ... . ... . ... . ....(Configuration);

                //Avvio della chiamata asincrona
                AsyncDocTypes(aooApi, docTypesApi);
                MessageBox.Show("Metodo asincrono iniziato il: " + DateTime.Now.ToString("O"));

                //Chiediamo all'utente se la chiamata asincrona si è completata?
                MessageBox.Show("Il metodo asincrono si è completato?");
            }
            catch (Exception exception)
            {
                errorLabel.Text = exception.Message;
