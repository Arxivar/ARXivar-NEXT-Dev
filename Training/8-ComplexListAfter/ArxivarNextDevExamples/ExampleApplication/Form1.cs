...
namespace ExampleApplication
{
    public partial class Form1 : Form
    {
        ...
        private void buttonPredefinedProfile_Click(object sender, EventArgs e)
        {
            try
            {
                //Inizializzo l'oggetto predefinedProfileApi da IO.Swagger.Api.PredefinedProfilesApi
                var predefinedProfileApi = new ... . ... . ... . ...(Configuration);
                
                //Recupero la lista dei profili predefiniti
                var predefProfiles = predefinedProfileApi....();

                //Creo una tabella riassuntiva
                datagridComplex.DataSource = ...;
        ...
        private void buttonPredProfById_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (datagridComplex.SelectedRows != null && datagridComplex.SelectedRows.Count > 0)
                {
                    //Recupero i profili predefiniti dalla riga selezionata
                    var predefinedProfileSelected = (PredefinedProfileDTO)datagridComplex.SelectedRows[0].DataBoundItem;

                    //Recupero i dettagli del profilo predefinito 
                    var predefinedProfileDetail = predefinedProfileApi. ...(predefinedProfileSelected.Id);

                    //Quali sono i campi del profilo predefinito?
                    MessageBox.Show(string.Format("Il profilo predefinito contiene {0} dettagli", predefinedProfileDetail. ... . ...));                    
                }
